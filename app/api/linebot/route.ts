...
export const POST = async (req: Request) => {
    ...
    // スプレッドシートの最初のシートを取得
    const sheet = doc.sheetsByTitle['result1'];
    ...
    for (const event of events) {
        if (event.type === 'message' && event.message.type === 'text') {
            ...
            if (message === '【慰謝料計算をする】') { //初回登録
                ...
                const row = rows.find(row => row.get('id') == '');
                ...
                if(row) {
                    ...
                    const profile = await getLineProfile(userId)
                    await sheet.addRow([profile.userId, 17, profile.displayName, ''])
                }
                ...
                if (currentMessageObject.conditions) {
                    ...
                    if (condition && condition.exception) {
                        ...
                        if(messageObject.id == 16 && row?.get('accidentType') == '死亡') {//診断結果を表示
                            const resultMessage = await getDeathConsulationMoneyText(userId);
                            replyLineMessage(resultMessage.messages, event.replyToken)
                        } else if(messageObject.id == 16)  {
